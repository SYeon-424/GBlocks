...
def auto_align_if_needed(entries: List[Tuple[str,str]], aligner="auto",
    ...
    if chosen == "auto":
        mafft_dir = os.path.join(os.path.dirname(__file__), "mafft-win")
...
def gblock_pipeline_safe(entries: List[Tuple[str,str]],
                         min_block_len=10, max_gap=0.5, min_cons=0.7, flank_cons:Optional[float]=None,
                         drop_all_gap_columns=True, max_noncons_run:int=0
                         ) -> List[Tuple[str,str]]:
    """
    어떤 입력을 넣어도 오류 대신 '최선의 결과'를 반환.
    """
    xs = safe_align(entries)                # ← 무조건 동일 길이 보장
    if not xs:
        return []                           # 비었으면 빈 결과

    try:
        return gblock_filter(xs, min_block_len, max_gap, min_cons, flank_cons,
                             drop_all_gap_columns, max_noncons_run)
    except Exception:
        # 만약 여기서도 실패한다면 파라미터를 느슨하게 낮춰서 한 번 더 시도
        try:
            return gblock_filter(xs, min(5, min_block_len), min(0.9, max_gap), min(0.5, min_cons),
                                 None, True, max(min_noncons_run if (min_noncons_run:=max_noncons_run) else 5, 5))
        except Exception:
            # 그래도 안 되면 그냥 equalize된 원본을 그대로 반환(트리밍 없이)
            return xs
