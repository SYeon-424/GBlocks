...
def _base_dir():
    """exe와 같은 폴더를 쓰려면: PyInstaller로 묶였을 때는 exe 위치, 아니면 스크립트 위치."""
...
def run_mafft(in_fa: str, out_fa: str, exe: Optional[str] = None):
    """
    Windows 배치(공백/한글 경로)까지 안전한 MAFFT 호출.
    - 프로젝트 내 mafft-win/(mafft|mafft.bat|mafft.cmd|mafft.exe) 우선
    - 배치/확장자 없는 실행파일은 shell=True
    - 배치파일이면 cwd를 해당 실행파일 폴더로 지정(상대경로 의존성 대응)
    """
    local_dir = _base_dir()
    candidates = [
        os.path.join(local_dir, "mafft-win", "mafft"),
        os.path.join(local_dir, "mafft-win", "mafft.bat"),
        os.path.join(local_dir, "mafft-win", "mafft.cmd"),
        os.path.join(local_dir, "mafft-win", "mafft.exe"),
    ]
    exe = exe or next((p for p in candidates if os.path.isfile(p)), None) or shutil.which("mafft")
    if not exe:
        raise RuntimeError("MAFFT not found (내장 mafft-win/* 또는 시스템 PATH 확인).")
    ...
    base = os.path.basename(exe_abs).lower()
    is_batch = (base.endswith(".bat") or base.endswith(".cmd") or base == "mafft")
    use_shell = is_batch
    workdir_for_mafft = os.path.dirname(exe_abs) if is_batch else (os.path.dirname(in_fa) or ".")

    cmd_list = [exe_abs, "--auto", "--quiet", in_fa]
    def _q(x: str) -> str:
        needs = (" " in x) or ("\t" in x) or any(ord(c) > 127 for c in x)
        return f'"{x}"' if needs else x
    cmd_str = " ".join(_q(x) for x in cmd_list)

    with open(out_fa, "w", encoding="utf-8") as fout:
        p = subprocess.run(
            cmd_str if use_shell else cmd_list,
            text=True,
            stdout=fout,
            stderr=subprocess.PIPE,
            cwd=workdir_for_mafft,
            shell=use_shell,
            creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0),
        )

    if p.returncode != 0 or not os.path.isfile(out_fa) or os.path.getsize(out_fa) == 0:
        raise RuntimeError(
            f"MAFFT failed (rc={p.returncode})\nexe={exe_abs}\ncwd={workdir_for_mafft}\ncmd={cmd_str}\n"
            f"STDERR:\n{p.stderr.strip()}"
        )
...
def run_muscle(in_fa: str, out_fa: str, exe: Optional[str] = None):
    ...
    if not exe:
        raise RuntimeError("MUSCLE not found.")
    cmd = [exe, "-align", in_fa, "-output", out_fa]
    p = subprocess.run(cmd, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if p.returncode != 0:
        raise RuntimeError(f"MUSCLE failed: {p.stderr.strip()}")
...
def expand_to_pattern(base_ref: str, aln_seq: str, target_gaps):
    ...
    for ch in aln_seq:
        if ch == "-":
            ...
            aln_base.append(base_ref[j_base])
            j_base += 1
    out = []
    i_aln = 0
    for k in range(L):
...
def center_star_align(entries: List[Tuple[str, str]], match=1, mismatch=-1, gap=-2) -> List[Tuple[str, str]]:
    ...
    for h, seq in entries:
        ...
        aligned_seqs.append(s_new)
    order = {h: i for i, (h, _) in enumerate(entries)}
    out = list(zip(aligned_headers, aligned_seqs))
    out.sort(key=lambda x: order[x[0]])
    return out

def auto_align_if_needed(
    entries: List[Tuple[str, str]],
    aligner="auto",
    aligner_exe=None,
    fallback_align=True,
) -> List[Tuple[str, str]]:
    if len(entries) <= 1 or all_equal_length(entries):
        return entries

    chosen = (aligner or "auto").lower()
    if chosen == "auto":
        mafft_dir = os.path.join(_base_dir(), "mafft-win")
        cand = [os.path.join(mafft_dir, n) for n in ("mafft", "mafft.bat", "mafft.cmd", "mafft.exe")]
        local_mafft = next((p for p in cand if os.path.isfile(p)), None)
        if local_mafft:
            chosen = "mafft"
            aligner_exe = local_mafft
        elif shutil.which("mafft"):
            chosen = "mafft"
        elif shutil.which("muscle"):
            chosen = "muscle"
        else:
            chosen = "none"
    ...
    try:
        if chosen == "mafft":
            with TemporaryDirectory() as w:
                inp = os.path.join(w, "in.fasta")
                outp = os.path.join(w, "out.fasta")
                write_fasta(entries, inp, wrap=0)
                run_mafft(inp, outp, exe=aligner_exe)
                aligned = parse_fasta(outp)
            ...
            with TemporaryDirectory() as w:
                ...
                outp = os.path.join(w, "out.fasta")
                write_fasta(entries, inp, wrap=0)
                run_muscle(inp, outp, exe=aligner_exe)
                aligned = parse_fasta(outp)

        elif fallback_align:
            aligned = center_star_align(entries)
        else:
            raise RuntimeError(
                "Sequences are unaligned and no aligner found. Install MAFFT or MUSCLE, or enable fallback_align."
            )

        import datetime
        import tempfile
        droot = os.path.join(tempfile.gettempdir(), "gblocks_debug",
                             datetime.datetime.now().strftime("%Y%m%d"))
        os.makedirs(droot, exist_ok=True)
        tag = datetime.datetime.now().strftime("%H%M%S_%f")

        dbg_fa = os.path.join(droot, f"aln_{tag}.fasta")
        dbg_len = os.path.join(droot, f"aln_{tag}.tsv")
        dbg_log = os.path.join(droot, "debug.log")

        write_fasta(aligned, dbg_fa, wrap=0)
        with open(dbg_len, "w", encoding="utf-8") as w:
            w.write("header\tlen\n")
            for h, s in aligned:
                w.write(f"{h}\t{len(s)}\n")
        with open(dbg_log, "a", encoding="utf-8") as w:
            w.write(f"[align-debug] chosen={chosen} fa={dbg_fa} lens={dbg_len}\n")

        L0 = len(aligned[0][1]) if aligned else 0
        if not all(len(s) == L0 for _, s in aligned):
            raise RuntimeError(
                "정렬 실패: 동일 길이가 아닙니다. (정렬기 호출/입력 확인)\n"
                f"자세한 기록: {dbg_len}"
            )
...
def column_metrics(
    ...
    for i in range(L):
        ...
        if progress_callback and ((i + 1) % step == 0 or i + 1 == L):
            progress_callback(i + 1, L)
    return mets
...
def soft_trim_block(b, mets, flank_cons, max_gap):
    s, e = b
    while s < e and not (mets[s]["gap_ratio"] <= max_gap and mets[s]["cons_ratio"] >= flank_cons):
        s += 1
    while e > s and not (mets[e - 1]["gap_ratio"] <= max_gap and mets[e - 1]["cons_ratio"] >= flank_cons):
        e -= 1
...
def gblock_filter(
    ...
    max_gap=0.5,
    min_cons=0.7,
    flank_cons: Optional[float] = None,
    ...
    mets = column_metrics(
        seqs,
        progress_callback=(lambda i, total: report(100 * i // total, "트리밍 중")) if progress_callback and L else None,
    )
    ...
    if flank_cons is not None:
        blocks = [
            tb for b in blocks
            if (tb := soft_trim_block(b, mets, flank_cons, max_gap)) and (tb[1] - tb[0]) >= min_block_len
        ]
...
def gblock_pipeline(
    ...
    if auto_align:
        report(0, "정렬 중")
        aln = auto_align_if_needed(
            entries, aligner=aligner, aligner_exe=aligner_exe, fallback_align=fallback_align
        )
        report(50, "트리밍 중")
    else:
        report(0, "트리밍 중")
        aln = entries
    if not all_equal_length(aln):
        raise ValueError("Alignment failed; sequences still unequal length.")

    def filter_report(pct: int, msg: str) -> None:
        if progress_callback:
            progress_callback(50 + pct * 50 // 100, msg)

    result = gblock_filter(
        aln, min_block_len, max_gap, min_cons, flank_cons, drop_all_gap_columns, max_noncons_run,
        progress_callback=filter_report,
    )
    return result
...
def convert_gblocks_params_to_internal(
    N: int,
...
def _clean_seq(s: str) -> str:
    ...
    # 허용 문자: DNA/AA + gap
    allowed = set("ACGTN-ABCDEFGHIKLMNPQRSTVWY*")
    return "".join(ch for ch in s if ch in allowed)

def _sanitize_entries(entries: List[Tuple[str, str]], min_len: int = 15) -> List[Tuple[str, str]]:
    out = []
    seen = set()
    for h, s in entries:
        hh = (h or "seq").strip() or "seq"
        ss = _clean_seq(s)
        if len(ss.replace("-", "").replace("*", "")) < min_len:
            continue
        key = (hh, ss)
        if key in seen:
            continue
...
def _force_equalize(entries: List[Tuple[str, str]]) -> List[Tuple[str, str]]:
    if not entries:
        return entries
    L = max(len(s) for _, s in entries)
    return [(h, (s + "-" * (L - len(s))) if len(s) < L else s) for h, s in entries]

def safe_align(entries: List[Tuple[str,str]]) -> List[Tuple[str,str]]:
    """
    무조건 동일 길이의 다중서열을 돌려준다.
    1) 정리/필터 → 2) MAFFT → 3) MUSCLE → 4) center-star → 5) 강제 equalize
    """
    xs = _sanitize_entries(entries, min_len=15)
    if len(xs) <= 1:
        return xs  # 0/1개면 그대로

    # 1) 우선 try: auto (mafft/muscle/none)
    try:
        ys = auto_align_if_needed(xs, aligner="auto", fallback_align=True)
        if all_equal_length(ys): 
            return ys
    except Exception:
        pass
    ...
    # 2) MUSCLE 강제
    try:
        ys = auto_align_if_needed(xs, aligner="muscle", fallback_align=True)
        if all_equal_length(ys): 
            return ys
    except Exception:
        pass
    ...
    # 4) 최후: 길이 강제 보정(패딩)
    return _force_equalize(xs)

def gblock_pipeline_safe(
    entries: List[Tuple[str, str]],
    min_block_len=10,
    max_gap=0.5,
    min_cons=0.7,
    ...
    xs = safe_align(entries)
    if not xs:
        return []

    try:
        return gblock_filter(
            xs, min_block_len, max_gap, min_cons, flank_cons,
            drop_all_gap_columns, max_noncons_run
        )
    except Exception:
        try:
            mnr_val = max_noncons_run if max_noncons_run else 5
            return gblock_filter(
                xs, min(5, min_block_len), min(0.9, max_gap), min(0.5, min_cons),
                None, True, max(mnr_val, 5)
            )
        except Exception:
            return xs
...
def main():
    ...
    print(
        f"- Params: use_gblocks={args.use_gblocks_params} | min_block_len={min_block_len} "
        f"| max_gap={max_gap} | min_cons={min_cons} | flank_cons={flank_cons} | max_noncons_run={max_noncons_run}"
    )
    if args.use_gblocks_params:
        print(
            f"- Gblocks-style: mc={args.gb_min_cons_count}, mf={args.gb_flank_cons_count}, "
